...
Run `snapstack` any time for a status report (service + server health, update check), and `snapstack update` to bring
both the CLI and the server to the latest version (on Windows, in an Administrator terminal).
